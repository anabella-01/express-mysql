...
const qy= util.promisify(conexion.query).bind(conexion); //Colocar posterior a la conexión
...
//GET /categoria/:id para devolver una sola categoria
...
//POST /categoria para guardar una categoria nueva
app.post('/api/categorias', async (req,res)=>{

    try{
        //Valido que me envien la info correctamente
        if(!req.body.nombre){
            throw new Error('Faltan datos');
        }
        const nombre = req.body.nombre.toUpperCase();

        //Realizo la validación de categorias y asi evitar repeticiones.
        let query = 'SELECT id FROM genero WHERE nombre = ?';
        let respuesta = await qy(query, [nombre]);

        //Consulto si el array tiene o no elementos
...
app.post('/api/personas',async (req,res)=>{
    try{
        ...
        //toUpperCase devuelve el valor convertido en mayúsculas
        const nombre = req.body.nombre.toUpperCase();
...
app.put('/api/personas/:id', async (req,res)=>{
    ...
    try{
        //Validamos que se ingresen los datos requeridos
        ...
        //Si es correcto, realizamos la consulta a la BD 
        const nombre = req.body.nombre.toUpperCase();
        const apellido = req.body.apellido.toUpperCase();
        const alias = req.body.alias.toUpperCase();
        ...
        if(respuesta.length > 0){ //Si encuentra un dato 
...
//GET /libros para devolver todos los libros
 app.get('/api/libros', async(req,res)=>{
         ...
         try{
            ...
            let respuesta = await qy(query, [req.params.id, req.body.nombre, req.body.descripcion, req.body.id_genero, req.body.id_persona]);
            res.send({'respuesta': respuesta});
     }
     catch(e){
        console.error(e.message);
...
app.put('/api/libros/:id', async (req,res)=>{
    ...
    try{
        ...
        console.log(`Se modificaron los datos correctamente del libro`);
